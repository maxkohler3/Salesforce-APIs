...
Get OppID from URL
    ...
    ${URLOppID}=               Get Substring   ${SplitUrl}    -2    -1
    Log To Console             ${URLOppID}
    ${OppID0}                   Set Variable           ${URLOppID[0]}
    Log To Console             ${OppID0}
    [Return]                   ${OppID0}
...
Execute Apex Code
    QVision.SelectText         List<Opportunity>     reach=line
    QVision.HotKey             delete
    QVision.WriteText          List<Opportunity> records = [SELECT Id FROM Opportunity WHERE Id = '${OppId}'];
